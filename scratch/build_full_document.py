...
def build_full_document(output_path):
    ...
    add_bullet("Se observó que al ingresar repetidamente (clic rápido) sobre el botón de confirmación de venta, el sistema podía enviar múltiples peticiones antes de bloquear la interfaz, descontando el stock varias veces. Este defecto evidenció la falta de un estado de 'cargando' temporal en la UI.", bold_pre="Caja Negra: ")
    add_bullet("Al probar el registro de una venta, notamos que aunque el Trigger actualiza el inventario en la base de datos, el cliente Flutter requiere despachar eventos manuales para reflejar el estado inmediatamente en memoria, ya que no utiliza suscripciones Realtime.", bold_pre="Integración: ")
    add_bullet("Las evaluaciones revelaron que algunos mensajes emergentes (SnackBars) quedaban ocultos detrás del BottomSheet de ventas debido al z-index, requiriendo usar un Overlay global para mejorar la consistencia visual y el feedback.", bold_pre="Usabilidad: ")

    add_h2("8.2 Limitaciones de los Métodos Utilizados")
    add_bullet("La limitación principal del particionamiento de equivalencia es que se basa exclusivamente en la interfaz web, dependiendo de los tiempos de carga del DOM. Herramientas E2E no detectan errores de red silenciosos si el frontend no los expone gráficamente.", bold_pre="En Caja Negra: ")
    add_bullet("Las pruebas de API validan la respuesta del backend de forma aislada. La limitación radica en que no verifican si el cliente (Flutter) procesa bien el JSON, solo que el servidor responda de acuerdo a la documentación HTTP.", bold_pre="En Integración: ")
    add_bullet("Las herramientas automatizadas (Lighthouse) detectan problemas técnicos (contraste, ARIA) pero no evalúan flujos lógicos de negocio. El cuestionario SUS es subjetivo y aplicado a 3 usuarios no tiene alta significancia estadística.", bold_pre="En Usabilidad: ")
    ...
    add_bullet("No cubren la revisión de código interno (Caja Blanca), análisis de vulnerabilidades, ni validan qué pasa con la base de datos ante transacciones concurrentes de miles de usuarios simultáneos (Pruebas de Estrés).", bold_pre="Pruebas de Caja Negra: ")
    add_bullet("No cubren pruebas unitarias de funciones en Flutter, ni la validación del renderizado gráfico en el cliente. Tampoco verifican la resiliencia del sistema ante caídas de red o alta latencia.", bold_pre="Pruebas de Integración: ")
    add_bullet("No cubren la validez de la lógica de negocio ni la seguridad. Un usuario podría considerar el sistema 'muy fácil de usar' incluso si este no guarda la información correctamente, aspecto que escapa a la evaluación heurística o al cuestionario SUS.", bold_pre="Pruebas de Usabilidad: ")
